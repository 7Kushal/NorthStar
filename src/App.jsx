...
function EquityPanel({ trades, start, theme }) {
  ...
  const trendColor = closingBalance > openingBalance ? '#20c997' : closingBalance < openingBalance ? '#ef6673' : theme === 'dark' ? '#8a8d95' : '#4d83b3';
...
function MarketChart({ symbol, timeframe, setTimeframe, market, theme }) {
  ...
  const option = useMemo(() => {
    ...
    const up = '#16c784', down = '#ff5967', accent = theme === 'dark' ? '#20c997' : '#2f86ff';
    const mainSeries = chartType === 'candles' ? { name: symbol, type: 'candlestick', data: market.data.map(c => [c.open, c.close, c.low, c.high]), itemStyle: { color: up, color0: down, borderColor: up, borderColor0: down } } : { name: symbol, type: 'line', data: close, showSymbol: false, lineStyle: { color: accent, width: 2 }, areaStyle: { color: theme === 'dark' ? '#20c99718' : '#2f86ff12' } };
    mainSeries.markLine = market.price ? { symbol: 'none', silent: true, lineStyle: { color: accent, type: 'dashed', width: 1 }, label: { show: true, position: 'end', formatter: formatQuote(market.price), color: theme === 'dark' ? '#07140f' : '#fff', backgroundColor: accent, padding: [5, 8], borderRadius: 2 }, data: [{ yAxis: market.price }] } : undefined;
