...
function EquityPanel({ trades, start, theme }) {
  ...
  const openingBalance = Number(start) || 0;
  const dailyResults = sorted.reduce((days, trade) => {
    const day = localDateKey(trade.date);
    days.set(day, (days.get(day) || 0) + (Number(trade.pnl) || 0));
    return days;
  }, new Map());
  let running = openingBalance;
  const datedPoints = [...dailyResults.entries()].sort(([a], [b]) => a.localeCompare(b)).map(([day, pnl]) => ({
    name: new Date(`${day}T12:00:00`).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }),
    value: running += pnl,
  }));
  const points = [{ name: 'Opening', value: openingBalance }, ...datedPoints];
  const closingBalance = points.at(-1)?.value ?? openingBalance;
  const trendColor = closingBalance > openingBalance ? '#20c997' : closingBalance < openingBalance ? '#ef6673' : theme === 'dark' ? '#f59e0b' : '#4d83b3';
  const chartLine = theme === 'dark' ? '#292d38' : '#f0f1f5';
  const tooltipBackground = theme === 'dark' ? '#07101d' : '#ffffff';
  const tooltipBorder = theme === 'dark' ? '#1d3857' : '#dce5f2';
  const tooltipText = theme === 'dark' ? '#f7faff' : '#070b14';
  const option = { animation: false, grid: { left: 8, right: 8, top: 18, bottom: 8, containLabel: true }, tooltip: { trigger: 'axis', triggerOn: 'mousemove', alwaysShowContent: false, hideDelay: 80, enterable: false, confine: true, backgroundColor: tooltipBackground, borderColor: tooltipBorder, textStyle: { color: tooltipText, fontSize: 13.20 }, formatter: params => { const point = params?.[0]; return point ? `<b>${point.axisValueLabel}</b><br/><span style="color:${trendColor}">●</span>&nbsp; ${plainMoney(point.value)}` : ''; } }, xAxis: { type: 'category', boundaryGap: false, data: points.map(point => point.name), axisLabel: { color: '#84909c', fontSize: 13.20 }, axisLine: { lineStyle: { color: chartLine } }, splitLine: { show: false } }, yAxis: { type: 'value', scale: true, position: 'right', axisLabel: { color: '#84909c', fontSize: 13.20, formatter: v => Math.abs(v) >= 100000 ? `$${(v / 1000).toFixed(1)}k` : `$${Number(v).toLocaleString('en-US', { maximumFractionDigits: 0 })}` }, splitLine: { lineStyle: { color: chartLine } } }, series: [{ type: 'line', data: trades.length ? points.map(point => point.value) : [], showSymbol: true, symbolSize: 5, smooth: .22, lineStyle: { color: trendColor, width: 2.4 }, itemStyle: { color: trendColor, borderColor: theme === 'dark' ? '#07101d' : '#fff', borderWidth: 1.5 }, areaStyle: { color: { type: 'linear', x: 0, y: 0, x2: 0, y2: 1, colorStops: [{ offset: 0, color: `${trendColor}38` }, { offset: 1, color: `${trendColor}00` }] } } }] };
...
function MarketChart({ symbol, timeframe, setTimeframe, market, theme }) {
  ...
  const option = useMemo(() => {
    ...
    const up = '#16c784', down = '#ff5967', accent = theme === 'dark' ? '#f59e0b' : '#2f86ff';
    const mainSeries = chartType === 'candles' ? { name: symbol, type: 'candlestick', data: market.data.map(c => [c.open, c.close, c.low, c.high]), itemStyle: { color: up, color0: down, borderColor: up, borderColor0: down } } : { name: symbol, type: 'line', data: close, showSymbol: false, lineStyle: { color: accent, width: 2 }, areaStyle: { color: theme === 'dark' ? '#f59e0b18' : '#2f86ff12' } };
    mainSeries.markLine = market.price ? { symbol: 'none', silent: true, lineStyle: { color: accent, type: 'dashed', width: 1 }, label: { show: true, position: 'end', formatter: formatQuote(market.price), color: theme === 'dark' ? '#17100a' : '#fff', backgroundColor: accent, padding: [5, 8], borderRadius: 2 }, data: [{ yAxis: market.price }] } : undefined;
    return { animation: false, backgroundColor: 'transparent', axisPointer: { link: [{ xAxisIndex: [0, 1] }], lineStyle: { color: '#8d9098', type: 'dashed' }, label: { backgroundColor: '#4a4e57' } }, tooltip: { trigger: 'axis', triggerOn: 'mousemove|click', alwaysShowContent: true, hideDelay: 60000, enterable: true, confine: true, axisPointer: { type: 'cross' }, showContent: true, backgroundColor: theme === 'dark' ? '#07101d' : '#fff', borderColor: theme === 'dark' ? '#1d3857' : '#dce5f2', textStyle: { color: theme === 'dark' ? '#f7faff' : '#070b14', fontSize: 14.30 } }, grid: [{ left: 12, right: 72, top: 20, height: '66%', containLabel: true }, { left: 12, right: 72, top: '76%', height: '14%', containLabel: true }], xAxis: [{ type: 'category', data: labels, boundaryGap: true, axisLine: { lineStyle: { color: axisLine } }, axisLabel: { show: false }, splitLine: { show: true, lineStyle: { color: gridLine } }, min: 'dataMin', max: 'dataMax' }, { type: 'category', gridIndex: 1, data: labels, boundaryGap: true, axisLine: { lineStyle: { color: axisLine } }, axisLabel: { color: '#8d9098', fontSize: 14.30, hideOverlap: true }, splitLine: { show: true, lineStyle: { color: gridLine } }, min: 'dataMin', max: 'dataMax' }], yAxis: [{ type: 'value', scale: true, min: dataLow - pricePadding, max: dataHigh + pricePadding, position: 'right', axisLabel: { color: '#8d9098', fontSize: 14.30, formatter: value => value.toFixed(symbol === 'XAUUSD' ? 2 : 0) }, axisLine: { show: true, lineStyle: { color: axisLine } }, splitLine: { lineStyle: { color: gridLine } } }, { type: 'value', gridIndex: 1, scale: true, show: false, splitLine: { show: false } }], dataZoom: [{ type: 'inside', xAxisIndex: [0, 1], start, end: 100, zoomOnMouseWheel: true, moveOnMouseMove: true, moveOnMouseWheel: true }], series: [mainSeries, ...(indicator ? [{ name: 'SMA 20', type: 'line', data: sma, showSymbol: false, smooth: true, lineStyle: { width: 1.5, color: '#d6a847' }, connectNulls: true }] : []), { name: market.volumeLabel, type: 'bar', xAxisIndex: 1, yAxisIndex: 1, data: market.data.map(c => ({ value: c.volume, itemStyle: { color: c.close >= c.open ? '#16c78444' : '#ff596744' } })), barWidth: '62%' }] };
...
function PerformanceCharts({ trades, theme }) {
  ...
  const tooltip = { triggerOn: 'mousemove', alwaysShowContent: false, hideDelay: 80, enterable: false, confine: true, backgroundColor: theme === 'dark' ? '#07101d' : '#fff', borderColor: theme === 'dark' ? '#1d3857' : '#dce5f2', textStyle: { color: ink, fontSize: 16.50 } };
  const analyticsAccent = theme === 'dark' ? '#f59e0b' : '#2f86ff';
  const equityOption = { animation: false, tooltip: { ...tooltip, trigger: 'axis', valueFormatter: value => money(value) }, grid: { left: 16, right: 16, top: 24, bottom: 28, containLabel: true }, xAxis: { type: 'category', boundaryGap: false, data: equity.map(point => point.date), axisLabel: { color: muted, fontSize: 13.20, hideOverlap: true }, axisLine: { lineStyle: { color: gridLine } } }, yAxis: { type: 'value', scale: true, axisLabel: { color: muted, fontSize: 13.20, formatter: value => `$${value}` }, splitLine: { lineStyle: { color: gridLine } } }, series: [{ name: 'Net P&L', type: 'line', data: equity.map(point => point.value), showSymbol: equity.length < 15, symbolSize: 7, smooth: .22, lineStyle: { color: analyticsAccent, width: 3 }, itemStyle: { color: analyticsAccent }, areaStyle: { color: { type: 'linear', x: 0, y: 0, x2: 0, y2: 1, colorStops: [{ offset: 0, color: theme === 'dark' ? '#f59e0b55' : '#2f86ff55' }, { offset: 1, color: theme === 'dark' ? '#f59e0b00' : '#2f86ff00' }] } }, markLine: { silent: true, symbol: 'none', label: { show: false }, lineStyle: { color: muted, opacity: .4 }, data: [{ yAxis: 0 }] } }] };
  const outcomeOption = { animation: false, tooltip: { ...tooltip, trigger: 'item', formatter: '{b}: {c} ({d}%)' }, legend: { bottom: 0, textStyle: { color: muted, fontSize: 13.20 } }, series: [{ type: 'pie', radius: ['55%', '76%'], center: ['50%', '44%'], avoidLabelOverlap: true, label: { color: ink, fontSize: 14.30, formatter: '{c}' }, labelLine: { lineStyle: { color: muted } }, data: outcomes }] };
  const activityOption = { animation: false, tooltip: { ...tooltip, trigger: 'axis', axisPointer: { type: 'shadow' }, formatter: params => { const index = params[0]?.dataIndex || 0, [date, value] = activity[index] || []; return `${date || ''}<br/>${value?.trades || 0} trade${value?.trades === 1 ? '' : 's'} · ${money(value?.pnl || 0)}`; } }, grid: { left: 12, right: 12, top: 22, bottom: 30, containLabel: true }, xAxis: { type: 'category', data: activity.map(([date]) => new Date(`${date}T12:00:00`).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })), axisLabel: { color: muted, fontSize: 13.20, hideOverlap: true }, axisLine: { lineStyle: { color: gridLine } }, axisTick: { show: false } }, yAxis: { type: 'value', scale: true, axisLabel: { color: muted, fontSize: 13.20 }, splitLine: { lineStyle: { color: gridLine } } }, series: [{ type: 'bar', barMaxWidth: 28, data: activity.map(([, value]) => ({ value: value.pnl, itemStyle: { color: value.pnl >= 0 ? '#20c997' : '#ef6673', borderRadius: value.pnl >= 0 ? [4, 4, 0, 0] : [0, 0, 4, 4] } })) }] };
...
function SessionAnalytics({ sessions, trades, theme }) {
  ...
  const option = {
    animation: false,
    color: ['#20c997', '#ef6673'],
    tooltip: { trigger: 'axis', triggerOn: 'mousemove|click', alwaysShowContent: true, hideDelay: 60000, enterable: true, confine: true, axisPointer: { type: 'shadow' }, backgroundColor: theme === 'dark' ? '#07101d' : '#fff', borderColor: theme === 'dark' ? '#1d3857' : '#dce5f2', textStyle: { color: theme === 'dark' ? '#f7faff' : '#070b14', fontSize: 14.30 } },
    legend: { top: 2, right: 4, textStyle: { color: theme === 'dark' ? '#93a1b5' : '#647084', fontSize: 13.20 } },
    grid: { left: 30, right: 12, top: 44, bottom: 32 },
    xAxis: { type: 'category', data: decisionProfile.map((_, index) => `Q${index + 1}`), axisTick: { show: false }, axisLine: { lineStyle: { color: theme === 'dark' ? '#20344e' : '#dce5f2' } }, axisLabel: { color: theme === 'dark' ? '#93a1b5' : '#647084', fontSize: 13.20 } },
    yAxis: { type: 'value', minInterval: 1, axisLabel: { color: theme === 'dark' ? '#748096' : '#8b95a6', fontSize: 13.20 }, splitLine: { lineStyle: { color: theme === 'dark' ? '#17273b' : '#e7edf5' } } },
...
function Breakdown({ title, eyebrow, data, theme }) {
  ...
  const option = { animation: false, tooltip: { trigger: 'axis', triggerOn: 'mousemove|click', alwaysShowContent: true, hideDelay: 60000, enterable: true, confine: true, axisPointer: { type: 'shadow' }, backgroundColor: theme === 'dark' ? '#07101d' : '#fff', borderColor: theme === 'dark' ? '#1d3857' : '#dce5f2', textStyle: { color: ink, fontSize: 14.30 }, formatter: params => { const item = items[params[0]?.dataIndex || 0]; return item ? `<b>${item[0]}</b><br/>${item[1].count} trade${item[1].count === 1 ? '' : 's'} · ${money(item[1].value)}` : ''; } }, grid: { left: 12, right: 36, top: 18, bottom: 20, containLabel: true }, xAxis: { type: 'value', axisLabel: { color: muted, fontSize: 13.20, formatter: value => `$${value}` }, axisLine: { lineStyle: { color: gridLine } }, splitLine: { lineStyle: { color: gridLine } } }, yAxis: { type: 'category', data: items.map(([name]) => name), axisLabel: { color: ink, fontSize: 14.30, width: 115, overflow: 'truncate' }, axisLine: { show: false }, axisTick: { show: false } }, series: [{ type: 'bar', barMaxWidth: 26, data: items.map(([, result]) => ({ value: result.value, itemStyle: { color: result.value >= 0 ? '#20c997' : '#ef6673', borderRadius: result.value >= 0 ? [0, 4, 4, 0] : [4, 0, 0, 4] }, label: { show: true, position: 'inside', color: '#fff', fontSize: 13.20, formatter: money(result.value) } })), markLine: { silent: true, symbol: 'none', label: { show: false }, lineStyle: { color: muted, opacity: .45 }, data: [{ xAxis: 0 }] } }] };
